...
- 可恢复会话：Redis 会话历史、上下文压缩、PostgreSQL 用户长期画像（配置/约束覆盖、偏好/历史增量追加）、语义记忆召回（BGE-M3 向量 + 词法回退）、停止生成和显式重试。
...
Next.js Chat / RAG Lab
  ...
  -> ChatService + Redis session memory + PostgreSQL long-term user profile
  -> LangGraph runtime
