...
规划器输出执行计划：单步，或 2-4 步依赖链（例如先搜新闻、再把文章标题注入规则问答）。

步骤间通过 `$ref:<output_key>.<field>` 传递上一步输出。
...
计划：[news:search -> regulation:ask]（第二步的 question 引用第一步文章标题）
...
裁判（ReAct reflector）观察工具结果：若工具失败、规则证据不足（`insufficient_evidence`）或新闻/赛况结果不完整，则生成 `next_plan` 重规划并回到步骤 5；否则合并检索信息到统一上下文。
